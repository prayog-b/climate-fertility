...
********************************************************************************
	...
	* Create log file 
	*log using "$cf/dm/logs/census/census_sanity_checks.log", replace
	ClimateAnalysisConfig, method("init")

	* Load dataset
	use "$cf/data/source/census/africa_women.dta", replace
...
********************************************************************************
* Generate variables for educational attainment
********************************************************************************
...
********************************************************************************
	...
	** Spouse industry 
	gen spouse_ind_none = (INDGEN_SP == 0) & !missing(INDGEN_SP)
	label variable spouse_ind_none "Spouse No industry (NA)"
	gen spouse_ind_agri = (INDGEN_SP == 10) & !missing(INDGEN_SP)   // This is our ag variable for spouse
	label variable spouse_ind_agri "Spouse Industry: Agriculture, fishing, forestry"
	gen spouse_ind_mine = (INDGEN_SP == 20) & !missing(INDGEN_SP)
	label variable spouse_ind_mine "Spouse Industry: Mining and extraction"
	gen spouse_ind_manuf = (INDGEN_SP == 30) & !missing(INDGEN_SP)
	label variable spouse_ind_manuf "Spouse Industry: Manufacturing"
	gen spouse_ind_utilities = (INDGEN_SP == 40) & !missing(INDGEN_SP)
	label variable spouse_ind_utilities "Spouse Industry: Electricity, water, gas, waste mgmt"
	gen spouse_ind_construct = (INDGEN_SP == 50) & !missing(INDGEN_SP)
	label variable spouse_ind_construct "Spouse Industry: Construction"
	gen spouse_ind_trade = (INDGEN_SP == 60) & !missing(INDGEN_SP)
	label variable spouse_ind_trade "Spouse Industry: Wholesale or retail trade"
	gen spouse_ind_hospitality = (INDGEN_SP == 70) & !missing(INDGEN_SP)
	label variable spouse_ind_hospitality "Spouse Industry: Hotels and restaurants"
	gen spouse_ind_logistics = (INDGEN_SP == 80) & !missing(INDGEN_SP)
	label variable spouse_ind_logistics "Spouse Industry: Transportation, storage, and communications"
	gen spouse_ind_finsurance = (INDGEN_SP == 90) & !missing(INDGEN_SP)
	label variable spouse_ind_finsurance "Spouse Industry: Financial services and insurance"
	gen spouse_ind_public = (INDGEN_SP == 100) & !missing(INDGEN_SP)
	label variable spouse_ind_public "Spouse Industry: Public admin and defense"
	gen spouse_ind_realestate = (INDGEN_SP == 111) & !missing(INDGEN_SP)
	label variable spouse_ind_realestate "Spouse Industry: Business services"
	gen spouse_ind_education = (INDGEN_SP == 112) & !missing(INDGEN_SP)
	label variable spouse_ind_education "Spouse Industry: Education"
	gen spouse_ind_health = (INDGEN_SP == 113) & !missing(INDGEN_SP)
	label variable spouse_ind_health "Spouse Industry: Health and social work"
	gen spouse_ind_other = (INDGEN_SP == 114) & !missing(INDGEN_SP)
	label variable spouse_ind_other "Spouse Industry: Other services"
	gen spouse_ind_hhservice = (INDGEN_SP == 120) & !missing(INDGEN_SP)
	label variable spouse_ind_hhservice "Spouse Industry: Private household services"
...
********************************************************************************
	...
	save "$cf/data/derived/census/africa-women-genvar.dta", replace
