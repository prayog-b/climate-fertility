...
BASE_DIR = "D:/NUS Dropbox/Prayog Bhattarai/Climate_Change_and_Fertility_in_SSA"
...
country_coordinates = {
    ...
    "Côte d'Ivoire": {"north": 11.5, "south": 4.3, "east": -2.4, "west": -8.7},
...
# write a function that will check the file name for the year under consideration
# The files are named like "Africa_1940", "Africa_1941" and so on. 
# This function would need to look at the file name and then extract that year and return that year. 
...
#     """Extract the year from a netCDF file by looking at the timestamps."""
...
def process_precipitation_file(nc_file, country_name, bbox, year):
    """
    Process precipitation data, resample to daily level, and save as CSV
    ...
    # Save to CSV
    output_path = os.path.join(OUTPUT_DIR, country_name, f"{country_name}_{year}_precip.csv")
    daily_precip.to_csv(output_path, index=False)
...
def process_temperature_file(nc_file, country_name, bbox, year):
    """
    Process temperature data, resample to daily level, and save as CSV
    ...
    # Save to CSV
    output_path = os.path.join(OUTPUT_DIR, country_name, f"{country_name}_{year}_temp.csv")
    daily_temp.to_csv(output_path, index=False)
...
def process_country_data(ds, country_name, bbox, year):
    """
    Process climate data for a specific country and save as CSV files
...
def process_precipitation_data(df, country_name, year):
    """Process precipitation data and save as CSV."""
...
def process_temperature_data(df, country_name, year):
    """Process temperature data and save as CSV."""
    # convert temperature data from Kelvin to Celsius
    df['temp']= df['t2m'] - 273.15

    # calculate dialy mean and max temperatures
