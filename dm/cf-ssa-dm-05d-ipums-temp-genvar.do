    ...
    * Get country name from code
    ...
    if "`method'" == "load_and_aggregate" {
        ...
        ** Create temperature bins 
        ...
        collapse (sum) `all_temp_vars' (mean) temp_mean (max) temp_max, by(SMALLEST year)
        recast float year
        ...
        * Create directory if it doesn't exist
        ...
        local save_file "$cf/data/derived/era/`country_name'/`country_name'_temperature_annual.dta"
...
program define CohortMetricsCalculator
    ...
    * Get all temperature variables
    ...
    collapse (sum) `temp_vars' (mean) temp_mean (max) temp_max, by(SMALLEST cohort)
    ...
    * Get country name from code
    ...
    if "`method'" == "run_full_analysis" {
        ...
        * Step 2: Process cohort metrics by census year
        ...
        foreach census_year in `census_years' {
            ...
            * Check cohort file
            ...
            local temp_file "$cf/data/derived/era/`country_name'/`country_name'_temperature_annual.dta"
        ...
        * Define age bins (must match generate_age_range_exposure)
        ...
        local final_file "$cf/data/derived/era/`country_name'/`country_name'_allcensus-cohort-temperature.dta"
