...
Output:
    - Creates file: $cf/data/derived/dhs/00-dhs-country-surveyyear-mappings/dhs-cohort-year.dta
    - Variables: cohort (birth year), year (calendar year), age (age in that year)
    - Only includes observations where age is between 15-44

Data Structure:
    - Cohorts: 1926-2009 (84 birth cohorts)
    - Years: 1940-2023 (84 calendar years)
...
Algorithm:
    ...
    2. Create cohort dataset (1926-2009)
    3. Create year dataset (1940-2023)
...
program define generate_cohort_year_pairs
    * Check if cohort-year pairs already exist
    capture confirm file "$cf/data/derived/dhs/00-dhs-country-surveyyear-mappings/dhs-cohort-year.dta"
    // if _rc == 0 {
    ...
    * keep cohorts where age is between 15 and 44
    ...
    save "$cf/data/derived/dhs/00-dhs-country-surveyyear-mappings/dhs-cohort-year.dta", replace
